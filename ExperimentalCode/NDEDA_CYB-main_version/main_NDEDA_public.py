...
def fit_train1(x1, train_data):
    ...
    for ii in range(len(x1)):
        x[0, ii] = x1[ii]
    x = random.choice(x)
    x = 1 * (x >= 0.6)  # Convert continuous values to binary 0 or 1 based on threshold
    
    if np.count_nonzero(x) == 0:
        f1 = 1  # error_rate (if no features are selected, error is maximized)
        f2 = 1  # feature size (0 features selected)
    else:
        x = "".join(map(str, x))
        value_position = np.array(list(find_all(x, '1'))) + 1  # Find positions of selected features
        value_position = np.insert(value_position, 0, 0)  # Insert the label column (first column)

        tr = train_data[:, value_position]  # Prepare training data by selecting the features
        
        # SVM with RBF kernel
        clf = SVC(kernel='rbf', random_state=None)  # Using the radial basis function (RBF) kernel
        scores = cross_val_score(clf, tr[:, 1:], tr[:, 0], cv=5)  # 5-fold cross-validation
        f1 = np.mean(1 - scores)  # Error rate (1 - accuracy)
        f2 = len(value_position) - 1  # Number of features selected

    f = f1 + 0.000001 * f2  # Fitness function: error rate + a small penalty for feature size
    return f
...
def proposed_mutation2(dis, offspring, ii, nei, gen, fit_num, Max_FES, strategy='NDEDA'):
    ss = np.argsort(dis) 
    niche_offspring = [offspring[t] for t in ss[1:nei]]
    member = offspring[ii]   
    
    s = [(member.fitness.value >= t.fitness.value)*1 + (member.fitness.value < t.fitness.value)*0 for t in niche_offspring] 
    sum_abs = sum(s) 
    
    f_mu = 0.6 # مقدار F (Scale Factor) که می‌توانید آن را تغییر دهید
    
    # پیدا کردن بهترین فرد کل جمعیت برای استراتژی‌های best
    pop_fit_whole = [ind.fitness.value for ind in offspring]
    min_idx_global = np.argmin(pop_fit_whole)
    best_global = offspring[min_idx_global]

    # انتخاب کاندیداها برای Mutation
    # حذف فرد فعلی از لیست انتخاب‌ها برای جلوگیری از انتخاب تکراری
    candidates = [ind for idx, ind in enumerate(offspring) if idx != ii]
    ...
    if strategy == 'DE/rand/1':
        ...
        for i in range(len(y_new)):
            y_new[i] = r1[i] + f_mu * (r2[i] - r3[i])

    elif strategy == 'DE/best/1':
        # V = X_best + F * (X_r1 - X_r2)
        r1, r2 = random.sample(candidates, 2)
        for i in range(len(y_new)):
            y_new[i] = best_global[i] + f_mu * (r1[i] - r2[i])

    elif strategy == 'DE/current-to-best/1':
        # V = X_i + F * (X_best - X_i) + F * (X_r1 - X_r2)
        r1, r2 = random.sample(candidates, 2)
        for i in range(len(y_new)):
            y_new[i] = member[i] + f_mu * (best_global[i] - member[i]) + f_mu * (r1[i] - r2[i])

    else: # حالت پیش‌فرض خود NDEDA
        niche_offspring1 = [offspring[t] for t in ss[:nei]] 
        pop_fit = [ind.fitness.value for ind in niche_offspring1]
        
        if sum_abs < 4:
            nbest = best_global
            offspring1 = [niche_offspring1[t] for t in range(len(niche_offspring1))]
        else:
            min_one = np.argmin(pop_fit)
            nbest = niche_offspring1[min_one]
            offspring1 = [offspring[t] for t in range(len(offspring))]
            
        if member in offspring1: offspring1.remove(member)
        if nbest in offspring1: offspring1.remove(nbest)
        
        in1, in2 = random.sample(offspring1, 2)
        for i in range(len(y_new)):
            y_new[i] = member[i] + f_mu * (nbest[i] - member[i]) + f_mu * (in1[i] - in2[i])
        return y_new, nbest

    return y_new, best_global # نهایتاً best_global را برای استفاده در محدودیت‌های Bound برمی‌گرداند
...
def main(seed,x_train,strategy):
    random.seed(None)
    random.seed(seed)
    ...
    min_fitness = []
    unique_number = []
    x_train = shuffle(x_train, random_state=None)
 ...
 # محاسبه فاصله‌ها (فاصله همینگ)    # محاسبه فاصله‌ها بین تمامی افراد     
    ...
    for gen in range(1, NGEN): # حلقه برای نسل‌های مختلف
        ...
        for ii in range(len(offspring)):#####upate the whole population  # بروزرسانی تمام افراد
            # # current_strategy = 'DE/best/1' # یا 'DE/rand/1' یا 'DE/current-to-best/1'
            # current_strategy = 'DE/rand/1' # یا 'DE/rand/1' یا 'DE/current-to-best/1'
            # # current_strategy = 'DE/current-to-best/1' # یا 'DE/rand/1' یا 'DE/current-to-best/1'
            y_new, nbest = proposed_mutation2(dis[ii, :], offspring, ii, nei, gen, fit_num, Max_FES, strategy=strategy)
...
if __name__ == "__main__":
    ...
    start = time.time()
    start = time.time()
    strategies = ['DE/rand/1', 'DE/best/1', 'DE/current-to-best/1']
    ...
    for strat in strategies:
        ...
        # اجرای الگوریتم
        pop, min_fitness, unique_number = main(seed, x_train, strategy=strat)
        
        # ذخیره فایل اصلی برای اسکریپت analyze_best.py
        strat_name = strat.replace('/', '_')
        EXA_array = np.array(pop)
        filename = f"{strat_name}_EXA_array_{dataset_name}.npy"
        np.save(filename, EXA_array)
        
        # --- بخش اصلاح شده برای ذخیره‌سازی جزئیات هر استراتژی ---
        # برای اینکه فایل‌های saveFile روی هم نوشته نشوند، نام دیتاست را با استراتژی ترکیب کنید
        custom_name = f"{dataset_name}_{strat_name}"
        
        pop1 = delete_duplicate(pop)
        pop_fit = [ind.fitness.value for ind in pop1]
        
        saveFile.saveAllfeature2(seed, custom_name, np.array(pop1))
        saveFile.saveAllfeature3(seed, custom_name, pop_fit)
        saveFile.saveAllfeature5(seed, custom_name, unique_number)
        saveFile.saveAllfeature6(seed, custom_name, min_fitness)
        # -------------------------------------------------------

    end = time.time()
    running_time = end - start
    # ذخیره زمان کل اجرا
    saveFile.saveAllfeature7(seed, dataset_name + "_TotalTime", running_time)
    print(f"\nAll strategies completed in {running_time:.2f} seconds.")
