...
def findindex(org, x):
    result = []
    for k,v in enumerate(org): #k和v分别表示org中的下标和该下标对应的元素 پیدا کردن اندیس‌های یک مقدار یا زیررشته
        if v == x:
            result.append(k)
    return result
...
def get_whole_01(individuals):
    #(کد برای تبدیل افراد به رشته‌های 0/1)    
    all_index = []
...
def whether_shown_before(a,all):
    # (کد برای تبدیل فرد a به رشته 0/1 و جستجو در لیست all)    
    b = np.array(a)
    x1 = 1 * (b >= 0.6)
    x1 = "".join(map(str, x1))
...
# همان قسمت الگوریتم 2 هست که میاد ان هئی تا ویژگی انتخاب شده و نشده را تغییر میده
def random_mutation(old):# چند بیت از الگوی انتخاب ویژگی‌اش را برعکس کند (۱→۰ و ۰→۱) تا زیرمجموعه ویژگی جدید بسازد.
    old1 = np.array(old) # تبدیل فرد به باینری
    old1 = 1 * (old1 >= 0.6)
    old1 = "".join(map(str, old1))
    select_position = np.array(list(find_all(old1, '1'))) # اندیس ویژگی‌های انتخاب شده --> 1
    non_position = np.array(list(find_all(old1, '0'))) # اندیس ویژگی‌های انتخاب نشده--> 0
    swap = min(int(0.5*len(select_position)),len(non_position)) # تعیین حداکثر تعداد تغییرات مجاز
    if swap == 0:
        swap = 1
    ...
    for i in temp1:
        old[i] = random.uniform(0,0.6) # ویژگی 1 را به [0, 0.6) (معادل 0) تغییر می‌دهد
    for j in temp2:
        old[j] =random.uniform(0.6,1) # ویژگی 0 را به [0.6, 1] (معادل 1) تغییر می‌دهد
    return old
...
def produce_unique_individuals(new,pop,dis,pop_unique,kk):
    ...
    for i in range(len(new)):
        ...
        if state != []:###shown before
            # while count < 2: #tau=2
            while count < TAU: #tau=2
                demo = new[i]
                new_solution = random_mutation(demo)
                #new_solution = DE_mutation(demo,niche_offspring)  ###from pop_non choose solution to mutate
                del demo
                for i_z in range(len(new_solution)):# بعد دوباره به باینری تبدیل می‌کند و چک می‌کند آیا این الگو جدید است یا نه:
                ...
                temp = findindex(unique_matrix_01, new_one_011) # اگر temp خالی بود یعنی جدید است:
                if len(temp) == 0:
                    new[i] = new_solution
            ...
            unique_matrix_01 = get_whole_01(pop_unique)
 #####################################################思想是继续check是否已经存在，已经存在就继续直到生成新的unique解The idea is to keep checking if a solution already exists; if it does, continue until a new unique solution is generated.
